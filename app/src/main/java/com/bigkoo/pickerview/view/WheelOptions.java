        ...
        wheelListener_option2 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                if (mOptions3Items != null) {
                    ...
                    opt3Selct = wv_option3.getCurrentItem();//上一个opt3的选中位置
                    //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                    opt3Selct = opt3Selct >= mOptions3Items.get(opt1Select).get(index).size() - 1 ? mOptions3Items.get(opt1Select).get(index).size() - 1 : opt3Selct;

                    wv_option3.setAdapter(new ArrayWheelAdapter(mOptions3Items
                            .get(wv_option1.getCurrentItem()).get(
                                    index)));
                    wv_option3.setCurrentItem(opt3Selct);

                }
                if (mOptions4Items != null) {
                    wheelListener_option3.onItemSelected(opt3Selct);
                }
            }
        };
        ...
        wheelListener_option3 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                if (mOptions4Items != null) {
                    ...
                    int opt2Select = opt1Select >= mOptions4Items.get(opt1Select).size() - 1 ? mOptions4Items.get(opt1Select).size() - 1 : opt1Select;
                    ...
                    int opt4 = wv_option4.getCurrentItem();//上一个opt3的选中位置
                    //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                    opt4 = opt4 >= mOptions4Items.get(opt1Select).get(opt2Select).get(index).size() - 1 ? mOptions4Items.get(opt1Select).get(opt2Select).get(index).size() - 1 : opt4;

                    wv_option4.setAdapter(new ArrayWheelAdapter(mOptions4Items
                            .get(wv_option1.getCurrentItem()).get(wv_option2.getCurrentItem()).get(
                                    index)));
